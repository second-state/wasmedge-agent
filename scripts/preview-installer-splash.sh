...
frames="${1:-${WASMEDGE_AGENT_SPLASH_PREVIEW_FRAMES:-300}}"
case "$frames" in
	""|*[!0-9]*)
...
preview_dir=$(mktemp -d "${TMPDIR:-/tmp}/wasmedge-agent-splash-preview.XXXXXX")
...
i=0
while [ "$i" -lt "$1" ]; do
	wasmedge_agent_screen "Installing WasmEdge Agent" "Downloading WasmEdge Agent$(wasmedge_agent_pulse)" "Fetching the verified package." ""
	sleep 0.18
	i=$((i + 1))
done
EOF
