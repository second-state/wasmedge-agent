...
<h1 align="center">WasmEdge Agent AI</h1>

<p align="center">
  LLM provider toolkit.
</p>

Release docs use the WasmEdge Agent package name. The source workspace manifest still keeps an inherited package name until the namespace migration is complete.
...
```bash
npm install wasmedge-agent-ai
```

TypeBox exports are re-exported from `wasmedge-agent-ai`: `Type`, `Static`, and `TSchema`.

## Quick Start

```typescript
import { Type, getModel, stream, complete, Context, Tool, StringEnum } from 'wasmedge-agent-ai';
...
```typescript
import { Type, Tool, StringEnum } from 'wasmedge-agent-ai';
...
```typescript
import { stream, validateToolCall, Tool } from 'wasmedge-agent-ai';
...
```typescript
import { readFileSync } from 'fs';
import { getModel, complete } from 'wasmedge-agent-ai';
...
```typescript
import { getModel, streamSimple, completeSimple } from 'wasmedge-agent-ai';
...
```typescript
import { getModel, stream } from 'wasmedge-agent-ai';
...
```typescript
import { getProviders, getModels, getModel } from 'wasmedge-agent-ai';
...
```typescript
import { Model, stream } from 'wasmedge-agent-ai';
...
```typescript
import { streamAnthropic, type AnthropicOptions } from 'wasmedge-agent-ai';
...
```typescript
import { getModel, complete, Context } from 'wasmedge-agent-ai';
...
```typescript
import { Context, getModel, complete } from 'wasmedge-agent-ai';
...
- OAuth login flows are not supported in browser environments. Use the `wasmedge-agent-ai/oauth` entry point in Node.js.
...
```typescript
import { getEnvApiKey } from 'wasmedge-agent-ai';
...
```bash
npx wasmedge-agent-ai login              # interactive provider selection
npx wasmedge-agent-ai login anthropic    # login to specific provider
npx wasmedge-agent-ai list               # list available providers
```
...
The library provides login and token refresh functions via the `wasmedge-agent-ai/oauth` entry point. Credential storage is the caller's responsibility.
...
} from 'wasmedge-agent-ai/oauth';
```

### Login Flow Example

```typescript
import { loginGitHubCopilot } from 'wasmedge-agent-ai/oauth';
...
```typescript
import { getModel, complete } from 'wasmedge-agent-ai';
import { getOAuthApiKey } from 'wasmedge-agent-ai/oauth';
...
- Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `wasmedge-agent-ai`
