...
<h1 align="center">WasmEdge Agent TUI</h1>
...
```typescript
import { TUI, Text, Editor, ProcessTerminal, matchesKey } from "wasmedge-agent-tui";
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "wasmedge-agent-tui";
...
```typescript
import { Container, type Focusable, Input } from "wasmedge-agent-tui";
...
Renders Kitty or iTerm2 terminal graphics when supported. Set `fallbackOnly: true` to show compact image metadata instead; WasmEdge Agent uses this metadata-only mode.
...
```typescript
import { CombinedAutocompleteProvider } from "wasmedge-agent-tui";
...
```typescript
import { matchesKey, Key } from "wasmedge-agent-tui";
...
```typescript
import { visibleWidth, truncateToWidth, wrapTextWithAnsi } from "wasmedge-agent-tui";
...
```typescript
import { matchesKey, Key, truncateToWidth } from "wasmedge-agent-tui";
import type { Component } from "wasmedge-agent-tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "wasmedge-agent-tui";
import type { Component } from "wasmedge-agent-tui";
