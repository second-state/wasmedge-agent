...
describe("tryExecUpdateRelaunch", () => {
	it("replaces the current process while preserving argv zero and the environment", () => {
		const environment = { WASMEDGE_AGENT_CODING_AGENT_DIR: "/tmp/agent", OMITTED: undefined };
		const chdir = vi.fn();
			...
			tryExecUpdateRelaunch(
				{
					...
					args: ["--trace-warnings", "/opt/wasmedge-agent/cli.js", "--resume", "session"],
				},
				{
					platform: "darwin",
					nodeVersion: "26.1.0",
		...
		expect(execve).toHaveBeenCalledWith(
			"/usr/bin/node",
			["/usr/bin/node", "--trace-warnings", "/opt/wasmedge-agent/cli.js", "--resume", "session"],
			{ WASMEDGE_AGENT_CODING_AGENT_DIR: "/tmp/agent" },
		);
