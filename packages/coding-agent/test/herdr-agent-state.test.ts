...
describe("herdrAgentStateExtension", () => {
	...
	it("reports lifecycle state to the herdr socket", async () => {
		...
		process.env.WASMEDGE_AGENT_CODING_AGENT_DIR = tempDir;
		...
		expect(requests[0]?.params.agent).toBe("wasmedge-agent");
		...
		expect(requests[3]?.params.agent).toBe("wasmedge-agent");
	});
