...
function toolResult(details: unknown, toolName = "rust"): ToolResultMessage {
...
describe("compaction file-op extraction", () => {
	...
	it("records cell-reported edits from rust tool results", () => {
		const fileOps = createFileOps();
		extractFileOpsFromMessage(
			toolResult({
				status: "ok",
	...
	it("ignores non-rust tool results and malformed diff payloads", () => {
		const fileOps = createFileOps();
	...
	it("caps cell file lists to keep summary blocks bounded", () => {
		const fileOps = createFileOps();
	...
	it("feeds cell edits into modified-files for summaries", () => {
		const fileOps = createFileOps();
		extractFileOpsFromMessage(
			toolResult({ diffs: [{ path: "src/cell-edit.ts", oldStr: "a", newStr: "b" }] }) as never,
			fileOps,
		);
		const { readFiles, modifiedFiles } = computeFileLists(fileOps);
		expect(readFiles).toEqual([]);
		expect(modifiedFiles).toEqual(["src/cell-edit.ts"]);
