...
describe("daemon command", () => {
	...
	it("cleans prompt listeners when the prompt request fails", async () => {
		await expect(
			handleDaemonCommand(["daemon", "--socket", "/tmp/wasmedge-agent.sock", "prompt", "active-1", "hello"]),
		).resolves.toBe(true);
	...
	it("chooses a terminating non-colliding default name past the safe-integer range", async () => {
		...
		await expect(handleDaemonCommand(["daemon", "--socket", "/tmp/wasmedge-agent.sock"])).resolves.toBe(true);
	...
	it("keeps bare --resume values as session id selectors", async () => {
		await expect(
			handleDaemonCommand(["daemon", "--socket", "/tmp/wasmedge-agent.sock", "create", "--resume", "abc123"]),
		).resolves.toBe(true);
	...
	it("rejects unknown send options instead of folding them into the message", async () => {
		await expect(
			handleDaemonCommand(["daemon", "--socket", "/tmp/wasmedge-agent.sock", "send", "worker", "--bogus", "hello"]),
		).resolves.toBe(true);
	...
	it("rejects extra agent-messages status arguments", async () => {
		await expect(
			handleDaemonCommand(["daemon", "--socket", "/tmp/wasmedge-agent.sock", "agent-messages", "pause", "active-1"]),
		).resolves.toBe(true);
	...
	it("resolves agent names before filtering scheduled prompts", async () => {
		...
		await expect(
			handleDaemonCommand(["daemon", "--socket", "/tmp/wasmedge-agent.sock", "--json", "cron", "list", "alpha"]),
		).resolves.toBe(true);
	...
	it("does not leak --goal/--goal-token-budget into daemon startup args", async () => {
		...
		await handleDaemonCommand([
			"daemon",
			"--socket",
			"/tmp/wasmedge-agent-goal-leak-test.sock",
			"start",
			"--goal",
			"Leak test goal",
			"--goal-token-budget",
	...
	it("does not leak goal into default config for a subsequent no-goal create", async () => {
		...
		await handleDaemonCommand(["daemon", "--socket", "/tmp/wasmedge-agent.sock", "create", "second"]);
	...
	it("errors on non-interactive daemon attach with the json remediation instead of hanging", async () => {
		...
		await expect(
			handleDaemonCommand(["daemon", "--socket", "/tmp/wasmedge-agent.sock", "attach", "active-1"]),
		).resolves.toBe(true);
	...
	it("prints the created session for non-interactive --json open instead of attaching", async () => {
		...
		await expect(handleDaemonCommand(["daemon", "--socket", "/tmp/wasmedge-agent.sock", "--json"])).resolves.toBe(
			true,
		);
