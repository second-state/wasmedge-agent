...
export const WASMEDGE_AGENT_BUILD_ID_ENV = "WASMEDGE_AGENT_BUILD_ID";
export const WASMEDGE_AGENT_LAUNCHER_PATH_ENV = "WASMEDGE_AGENT_LAUNCHER_PATH";
...
export function getDaemonRuntimeIdentity(environment: NodeJS.ProcessEnv = process.env): DaemonRuntimeIdentity {
	...
	const launcher = environment[WASMEDGE_AGENT_LAUNCHER_PATH_ENV];
	return {
		buildId: environment[WASMEDGE_AGENT_BUILD_ID_ENV] ?? bundledBuildId() ?? `release-${VERSION}`,
