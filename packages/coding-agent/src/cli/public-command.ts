	...
	switch (command) {
		...
		case "update": {
			...
			if (hasLegacySelfTarget && hasLegacyPackageTarget) {
				return fail(
					"WasmEdge Agent and package updates are now separate.",
...
function rejectRemovedCommand(args: string[]): PublicCommandResult {
	...
	if (command === "daemon") {
		replacement = 'Run "wasmedge-agent help" to see the agent commands.';
	} else if (command === "app" && subcommand === "update") {
		replacement = 'Use "wasmedge-agent update".';
	} else if (command === "install") {
		replacement = 'Use "wasmedge-agent package install".';
	} else if (command === "remove" || command === "uninstall") {
		replacement = 'Use "wasmedge-agent package remove".';
	} else if (command === "manage") {
		replacement = 'Use "wasmedge-agent agents".';
	}
...
async function runPackage(args: string[]): Promise<PublicCommandResult> {
	...
	if (subcommand === "update") {
		if (
			...
			return fail(
				'Package updates accept only an optional source. Use "wasmedge-agent update --force" to update WasmEdge Agent.',
			);
		}
		if (rest.length > 1) {
		...
		if (rest[0] && isSelfUpdateSource(rest[0])) {
			return fail('Use "wasmedge-agent update" to update WasmEdge Agent.');
		}
