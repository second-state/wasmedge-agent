...
/** Parse `ss -lxp` output into the wasmedge-agent daemons listening on unix sockets. */
...
export function parseWasmEdgeAgentProcessIds(stdout: string, appName: string): number[] {
	const pids: number[] = [];
	for (const line of stdout.split("\n")) {
	...
	return daemons.map((daemon): ReapAction => {
		...
		if (daemon.status === "unreachable") {
			if (!force || daemon.pid === undefined) {
				return { kind: "skip", daemon, reason: 'unreachable; use "wasmedge-agent shutdown --force" to stop it' };
			}
...
export async function runShutdownAll(json: boolean, force: boolean): Promise<void> {
	...
	switch (planShutdownConfirmation(daemons.length, json, force, process.stdin.isTTY)) {
				...
				JSON.stringify(
					{
						...
						failed: daemons.map(({ socketPath }) => ({
							socketPath,
							reason: 'confirmation required; use "wasmedge-agent shutdown --force --json"',
						})),
					},
					null,
					2,
		...
		case "tty-error":
			throw new Error(
				'Shutdown requires confirmation in an interactive terminal. Use "wasmedge-agent shutdown --force".',
			);
		case "prompt": {
