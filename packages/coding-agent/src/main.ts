...
const STARTUP_SESSION_LOSS_COPY: DaemonSessionLossCopy = {
	busyDetail(count) {
		...
		return `A background service from a different WasmEdge Agent version is running with ${count} busy ${noun}. Stopping it will terminate ${pronoun}.`;
	},
	unlistableDetail:
		"A background service from a different WasmEdge Agent version is running and its sessions could not be listed. Stopping it may terminate active sessions.",
	question: "Stop it and continue?",
	nonTtyHint: 'Run "wasmedge-agent shutdown" to stop it, then retry.',
};
