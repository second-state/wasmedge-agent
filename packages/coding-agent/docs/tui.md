> WasmEdge Agent can create TUI components. Ask it to build one for your use case.
...
WasmEdge Agent shows compact image metadata and does not emit terminal graphics for images.
...
Customize the inline working indicator shown while WasmEdge Agent is streaming a response.
...
// Restore WasmEdge Agent's default spinner
ctx.ui.setWorkingIndicator();
```
