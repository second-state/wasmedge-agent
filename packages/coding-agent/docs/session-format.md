...
```
~/.wasmedge-agent/sessions/<session-id>.jsonl
```
...
Sessions can be removed by deleting their `.jsonl` files under `~/.wasmedge-agent/sessions/`.

WasmEdge Agent also supports deleting sessions interactively from `/resume` (select a session and press `Ctrl+D`, then confirm). When available, WasmEdge Agent uses the `trash` CLI to avoid permanent deletion.
...
### Base Message Types (from WasmEdge Agent AI)
...
### Extended Message Types (from WasmEdge Agent Coding Agent)
...
- `fromHook`: `true` if generated by an extension, `false`/`undefined` if generated by WasmEdge Agent (legacy field name)
