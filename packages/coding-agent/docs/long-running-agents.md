...
Automatic compaction handles context growth during long tasks. On overflow or near the configured threshold, Prime Agent summarizes older messages, retains recent context, and continues. The cell workspace persists through compaction, so `rlm::state` entries, blobs, and `agent_lib` helper functions remain available.
