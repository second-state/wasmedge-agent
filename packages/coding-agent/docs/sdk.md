> WasmEdge Agent can help you use the SDK. Ask it to build an integration for your use case.

# SDK

The SDK provides programmatic access to WasmEdge Agent's capabilities. Use it to embed WasmEdge Agent in other applications, build custom interfaces, or integrate with automated workflows.
...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.wasmedge-agent", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.wasmedge-agent/extensions/`)
- Project skills:
  - `.wasmedge-agent/skills/`
  - `.agents/skills/` in `cwd` and ancestor directories (up to git repo root, or filesystem root when not in a repo)
- Project prompts (`.wasmedge-agent/prompts/`)
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.wasmedge-agent/skills/`)
  - `~/.agents/skills/`
...
// Default: uses ~/.wasmedge-agent/auth.json and ~/.wasmedge-agent/models.json
const authStorage = AuthStorage.create();
const modelRegistry = ModelRegistry.create(authStorage);
...
```typescript
// Use the default built-in tool set: rust, bash
const { session } = await createAgentSession({
  tools: ["rust"],
});
...
- If you omit `tools`, WasmEdge Agent automatically creates them with the correct `cwd`
...
Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.wasmedge-agent/extensions/`, `.wasmedge-agent/extensions/`, and `settings.json` extension sources.
...
1. Global: `~/.wasmedge-agent/settings.json`
2. Project: `<cwd>/.wasmedge-agent/settings.json`
...
```bash
wasmedge-agent --mode rpc --no-session
```
...
createRustTool, createBashTool, createEditTool
createRustToolDefinition, createBashToolDefinition, createEditToolDefinition
