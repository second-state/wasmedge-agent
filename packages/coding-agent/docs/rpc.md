...
```bash
wasmedge-agent --mode rpc [options]
```
...
```json
{
  ...
  "data": {
    "commands": [
      {"name": "session-name", "description": "Set or clear session name", "source": "extension", "path": "/home/user/.wasmedge-agent/extensions/session.ts"},
      {"name": "fix-tests", "description": "Fix failing tests", "source": "prompt", "location": "project", "path": "/home/user/myproject/.wasmedge-agent/prompts/fix-tests.md"},
      {"name": "skill:brave-search", "description": "Web search via Brave API", "source": "skill", "location": "user", "path": "/home/user/.wasmedge-agent/skills/brave-search/SKILL.md"}
    ]
  }
}
```
...
- `location`: Where it was loaded from (optional, not present for extensions):
  - `"user"`: User-level (`~/.wasmedge-agent/`)
  - `"project"`: Project-level (`./.wasmedge-agent/`)
...
```json
{
  ...
  "id": "uuid-8",
  "method": "setTitle",
  "title": "WasmEdge Agent - my project"
}
```
...
const agent = spawn("wasmedge-agent", ["--mode", "rpc", "--no-session"]);
