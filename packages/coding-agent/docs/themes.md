> WasmEdge Agent can create themes. Ask it to build one for your setup.
...
WasmEdge Agent loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.wasmedge-agent/themes/*.json`
- Project: `.wasmedge-agent/themes/*.json`
...
On first run, WasmEdge Agent detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.wasmedge-agent/themes
vim ~/.wasmedge-agent/themes/my-theme.json
```
...
```json
{
  "$schema": "https://raw.githubusercontent.com/PrimeIntellect-ai/wasmedge-agent/main/packages/coding-agent/src/modes/interactive/theme/theme-schema.json",
  "name": "my-theme",
  "vars": {
    "primary": "#00aaff",
    "secondary": 242
...
**Hot reload:** When you edit the currently active custom theme file, WasmEdge Agent reloads it automatically for immediate visual feedback.
...
WasmEdge Agent uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, WasmEdge Agent falls back to the nearest approximation.
