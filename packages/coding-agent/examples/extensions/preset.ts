 ...
 * - ~/.wasmedge-agent/presets.json (global)
 * - <cwd>/.wasmedge-agent/presets.json (project-local)
 *
...
function loadPresets(cwd: string): PresetsConfig {
	...
	const projectPath = join(cwd, ".wasmedge-agent", "presets.json");
...
export default function presetExtension(pi: ExtensionAPI) {
	...
	async function showPresetSelector(ctx: ExtensionContext): Promise<void> {
		...
		if (presetNames.length === 0) {
			ctx.ui.notify("No presets defined. Add presets to ~/.wasmedge-agent/presets.json", "warning");
			return;
		}
