...
Example extensions for WasmEdge Agent.

## Usage

```bash
# From the repository root, load an extension with --extension
./wasmedge-agent.sh --extension packages/coding-agent/examples/extensions/permission-gate.ts

# Or copy to extensions directory for auto-discovery
cp permission-gate.ts ~/.wasmedge-agent/extensions/
```
...
| Extension | Description |
|-----------|-------------|
| `mac-system-theme.ts` | Syncs the WasmEdge Agent theme with macOS dark/light mode |
